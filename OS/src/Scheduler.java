...
public class Scheduler {
    ...
    public static void schedule() {
        ...
        while (true) {
            ...
            int lowerBoundary = getLowerBoundary(currentProcess);
            String instruction = (String) memory[lowerBoundary + pc];
    ...
    private static int getLowerBoundary(Process currentProcess) {
        Object[] memory = Memory.getInstance().getMemory();
        int start = getPCBstart(currentProcess.getPid());
        String boundaries = (String) memory[start + 3];
        int lowerBoundary = Integer.parseInt(boundaries.split("-")[0]);
    ...
    public static void saveProcessToDisk(int last, int i , int index) {
        ...
        else if (processesInMemory.get(1).getPid() == pid && processesInMemory.get(1).getTempVar() != null)
